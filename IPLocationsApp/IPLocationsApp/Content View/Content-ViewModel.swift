...
extension ContentView {
    final class ViewModel:  ObservableObject {
        ...
        private func fetchIP(){
            api.fetchData(url: "https://api.ipify.org?format=json", model: IP.self) { result in
                self.runOnMain {
                    ...
                    print("IP Adress:", self.ipAddress)
        ...
        private func fetchLocation(ip: String){
            api.fetchData(url: "https://ipinfo.io/\(ip)/json/", model: IPCoordinates.self) { result in
                self.runOnMain {
